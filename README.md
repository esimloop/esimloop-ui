...
| `scripts/audit-contrast.js` | Console snippet that measures every text/background pair. |
...
`npm run check` proves the skeleton reads tokens and that the themes agree. It
cannot tell you whether the result is **readable**.

`scripts/audit-contrast.js` answers that one: paste it into the console, call
`auditContrast()`, and it measures every text/background pair on the page —
resolving `oklab()`/`oklch()` (which is what `color-mix()` computes to) and
compositing translucent text over the real stack of ancestor backgrounds, because
every text tier here is the ink at an opacity.

Run it in **both** themes. It was written after a reader spotted near-black text
on a near-black surface that every automated check had passed, and it then found
seventeen more groups of the same class of fault.
...
behaviour rather than merely rendered. Contrast was measured across the whole
reference page in both themes: the only remaining failures are the two tier sample
rows the page deliberately renders in `--text-subtle` and `--text-faint`, labelled
"large only" and "never text". On
